...
use windows::Win32::{
    Foundation::{CloseHandle, CHAR, FILETIME, HANDLE, HINSTANCE, SYSTEMTIME},
    System::{
        Diagnostics::ToolHelp::{
            CreateToolhelp32Snapshot, Process32First, Process32Next, PROCESSENTRY32,
            TH32CS_SNAPPROCESS,
        },
        ProcessStatus::K32GetModuleFileNameExA,
        Threading::{GetProcessTimes, OpenProcess, PROCESS_QUERY_INFORMATION},
        Time::FileTimeToSystemTime,
    },
};

pub fn is_any_running(
    exe_path: &Vec<String>,
    seen_paths: &mut Vec<(String, NaiveDateTime)>,
) -> Option<()> {
    ...
    unsafe {
        ...
        let get_as_string = |x: &[CHAR]| {
            CStr::from_bytes_until_nul(&x.iter().map(|c| c.0).collect::<Vec<u8>>())
                .unwrap()
                .to_str()
                .unwrap()
                .to_string()
        };
        let get_as_string_u8 = |x: &[u8]| {
        ...
        let mut are_we_chillin =
            |x: &str, mod32: &mut PROCESSENTRY32, handles: &mut Vec<(String, NaiveDateTime)>| {
                if !exe_path.iter().any(|b| b.ends_with(x)) {
                    ...
                    if handles.iter().any(|x| (*x).0 == actual_name)
                        || seen_paths.iter().any(|x| (*x).0 == actual_name)
                    {
                        CloseHandle(h);
        ...
        if let Some(pogging) =
            are_we_chillin(&get_as_string(&mod32.szExeFile), &mut mod32, &mut handles)
        {
