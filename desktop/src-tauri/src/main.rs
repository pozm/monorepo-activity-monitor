...
use std::{thread, time::Duration, sync::{Arc, Mutex, RwLock}, os::windows::raw::HANDLE, collections::{HashMap, HashSet}, fs::File, io::{BufReader, Read}};

use activity::*;
use chrono::{NaiveDateTime, Utc};
use lazy_static::lazy_static;
use activity_manager::{data_handler, user_settings::{USER_SETTINGS, ApplicationData},OkRequest, get_name_from_loc, SEEN_LOCAL, SEEN};
use reqwest::{header::HeaderMap, multipart::{Form, Part}, Client};
use serde_json::json;
use tauri::{SystemTrayMenu, SystemTray, SystemTrayEvent, Manager, Runtime, RunEvent, WindowEvent};
...
async fn main() {
  ...
  .invoke_handler(tauri::generate_handler![
    ...
    tokio::spawn( async move {
      loop {
        let old : HashSet<SeenData> = HashSet::from_iter(SEEN.read().unwrap().iter().cloned());
        let mut watch = vec![];
        {
          let us = USER_SETTINGS.clone();
          let set = us.read().unwrap();
          println!("{:?}",set);
          watch = set.applications.iter().map(|x|x.location.clone()).collect::<Vec<_>>();
        }
        is_any_running(&watch,&mut SEEN.write().unwrap());
        let new: HashSet<SeenData> = HashSet::from_iter(SEEN.read().unwrap().iter().cloned());
        ...
        w.emit("backend://activity-update", sl.clone());
...
#[tauri::command]
async fn new_application<R: Runtime>(app: tauri::AppHandle<R>, window: tauri::Window<R>,location:String,name:String,icon:String) -> Result<(), String> {
  let server;
  {
    let mut set = USER_SETTINGS.clone();
